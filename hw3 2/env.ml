...
(* We define an abstraction to represent environments, which are mappings from
   names (strings) to items of some unknown type.  This abstraction can be
...
module type ENV = sig
  ...
  val combine_envs: 'a env -> 'a env -> 'a env
    (* Combine a list of environments, with later ones shadowing bindings
       in earlier ones.  *)
  val combine_all_envs: 'a env list -> 'a env
...
module Env : ENV = struct
  ...
  (* Combine a list of environments, with later ones shadowing bindings
     in earlier ones.  *)
  let combine_all_envs (es:'a env list):'a env =
    List.fold_left combine_envs (empty_env()) es
