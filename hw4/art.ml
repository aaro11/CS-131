...
module Art = struct
  ...
  *)
  let rec forLoop (low,high,f) =
   (* let x = low in *)
    if high >= low
    then (f (low); (forLoop ((low + 1), high, f)))
    else ()
  ...
  *)
  let rec build depth =
	...
	(let rand = (Random.int 4) in
	 match rand with
	     0 -> Expr.buildSin (build (depth-1))
	   | 1 -> Expr.buildCos (build (depth-1))
	   | 2 -> Expr.buildAvg (build (depth-1), build (depth-1))
	   | _ -> Expr.buildTimes (build (depth-1), build (depth-1)))
